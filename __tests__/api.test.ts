...
describe('oauthToken', () => {
  ...
  it('calls oauth/token with the correct url', async () => {
    ...
    expect(mockFetch).toBeCalledWith('https://test.com/access/oauth/token', {
  ...
  it('calls oauth/token with a worker with the correct url', async () => {
    ...
    expect(spy).toHaveBeenCalledWith(
      {
        fetchUrl: 'https://test.com/access/oauth/token',
        fetchOptions: {
          body: JSON.stringify(body),
          headers: {
            'Content-Type': 'application/json',
  ...
  it('handles error with error response', async () => {
    ...
    try {
      await oauthToken({
        ...
        code_verifier: 'code_verifierIn',
        grant_type: 'authorization_code',
        earthoOneClient: DEFAULT_EARTHO_CLIENT
      });
    } catch (error) {
  ...
  it('handles error without error response', async () => {
    ...
    try {
      ...
      expect(error.message).toBe(
        `HTTP error. Unable to fetch https://test.com/access/oauth/token`
      );
