...
export interface AuthConfig {
   ...
   * "Allowed Callback URLs" field in your Eartho Application's
   ...
   * an Eartho application and attempt to exchange the code for a token.
   ...
   *
   * Read more about [creating a custom cache](https://github.com/eartho/one-client-js#creating-a-custom-cache)
   */
  cache?: ICache;

  /**
   * If true, refresh tokens are used to fetch new access tokens from the Eartho server.
   ...
   * the user to login interactively with `connectWithRedirect` or `connectWithPopup`."
   *
   * E.g. Using the `file:` protocol in an Electron application does not support that legacy technique.
   *
   *  this.authService.connectSilently().subscribe({
   *    error: (e) => {
   *      if (e.error === 'missing_refresh_token' || e.error === 'invalid_grant') {
   *        this.authService.connectWithRedirect();
   *      }
   *    }
   *  });
   */
...
/**
 * Gets and sets configuration for the internal Eartho client. This can be
 ...
 * // app.module.ts
 * // ---------------------------
 * import { AuthModule, AuthClientConfig } from '@eartho/one-client-angular';
 *
