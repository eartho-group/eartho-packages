...
import { AuthService } from 'projects/one-client-angular/src/lib/auth.service';
...
describe('AppComponent', () => {
  ...
  beforeEach(() => {
    authMock = jasmine.createSpyObj(
      'AuthService',
      {
        connectWithRedirect: jasmine.createSpy().and.returnValue(null),
        connectWithPopup: jasmine.createSpy().and.returnValue(null),
        logout: jasmine.createSpy().and.returnValue(null),
        connectSilently: jasmine.createSpy().and.returnValue(null),
  ...
  describe('when initialized', () => {
    ...
    it('should render title', () => {
      ...
      expect(h1?.textContent).toContain('EARTHO ANGULAR PLAYGROUND');
    });
  ...
  describe('when user is authenticated', () => {
    beforeEach(() => {
      const loading = authMock.isLoading$ as BehaviorSubject<boolean>;
      const authenticated = authMock.isConnected$ as BehaviorSubject<boolean>;
    ...
    it('should get access token silently', () => {
      ...
      (authMock.connectSilently as jasmine.Spy).and.returnValue(
      ...
      expect(authMock.connectSilently).toHaveBeenCalledWith({
        ignoreCache: false,
      });
  ...
  describe('when user is not authenticated', () => {
    ...
    it('should login with redirect', () => {
      ...
      expect(authMock.connectWithRedirect).toHaveBeenCalledWith({
        appState: {
    ...
    it('should login with popup', () => {
      ...
      expect(authMock.connectWithPopup).toHaveBeenCalledWith({});
    });
  });
});
