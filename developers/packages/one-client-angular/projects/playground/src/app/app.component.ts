...
import { LogoutOptions } from '@eartho/one-client-js';
...
export class AppComponent implements OnInit {
  ...
  access_id = '2drlTkv19Alfvu9pEPTP'; //Change Here
  ...
  launchLogin(): void {
    ...
    if (usePopup) {
      this.auth.connectWithPopup({
        access_id: this.access_id
      });
    } else {
      this.auth.connectWithRedirect({
        access_id: this.access_id,
        appState: {
  ...
  updateAccessToken(): void {
    ...
    iif(
      ...
      this.auth.connectSilently({ ignoreCache })
    )
      .pipe(first())
      .subscribe((token) => {
