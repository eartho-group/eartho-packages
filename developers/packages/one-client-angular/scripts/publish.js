...
if (!fs.existsSync(unpublishedFilePath)) {
  ...
  console.log(
    `If the Release PR was already merged into the base branch and only the npm publication is pending, manually run 'npm run build:prod' and then 'ALLOWED=true npm publish ./dist/one-client-angular'.`
  );
  process.exit(1);
}
...
(async () => {
  ...
  */
  ...
  await exec('cp README.md ./dist/one-client-angular/');

  console.log('Uploading to npm, you might be prompted for the OTP...');

  execSync(`ALLOWED=true npm publish ./dist/one-client-angular`, {
    stdio: 'inherit',
  });
