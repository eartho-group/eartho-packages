...
public class AuthenticationException : EarthoException {
    ...
    public constructor(message: String, cause: EarthoException? = null) : super(message, cause)
    ...
    public constructor(values: Map<String, Any>, statusCode: Int = 0) : this(DEFAULT_MESSAGE) {
        ...
        if (description is String) {
            ...
            val pwStrengthParser =
                PasswordStrengthErrorParser(
                    description as Map<String, Any>
                )
    ...
    private fun warnIfOIDCError() {
        if ("invalid_request" == getCode() && (ERROR_OIDC_ACCESS_TOKEN == getDescription() || ERROR_OIDC_RO == getDescription())) {
            Log.w(
                ...
                "Your Eartho Application is configured as 'OIDC Conformant' but this instance it's not. To authenticate you will need to enable the flag by calling Eartho#setOIDCConformant(true) on the Eartho instance you used in the setup."
            )
        }
    }
    ...
    public val isBrowserAppNotAvailable: Boolean
        get() = "eartho.browser_not_available" == code
    ...
    public val isMultifactorCodeInvalid: Boolean
        get() = "eartho.mfa_invalid_code" == code || "invalid_grant" == code && "Invalid otp_code." == description
    ...
    /// When Eartho rule returns an error. The message returned by the rule will be in `description`
    ...
    /// When authenticating with web-based authentication using prompt=none and the Eartho session had expired
    ...
    internal companion object {
        internal const val ERROR_VALUE_AUTHENTICATION_CANCELED = "eartho.authentication_canceled"
