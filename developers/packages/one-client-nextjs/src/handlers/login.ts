...
export interface AuthorizationParams extends Partial<AuthorizationParameters> {
  ...
  /**
   * Provides a hint to Eartho as to what flow should be displayed. The default behavior is to show a
...
const appRouteHandlerFactory: (
  ...
  async (req, _ctx, options = {}) => {
    try {
      ...
      await handler(earthoReq, earthoRes, applyOptions(req, options, dangerousReturnTo, config) as BaseLoginOptions);
