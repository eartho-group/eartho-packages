...
import type { TokenEndpointResponse } from '../eartho-session/client/abstract-client';
...
export function fromTokenEndpointResponse(tokenEndpointResponse: TokenEndpointResponse, config: NextConfig): Session {
  ...
  const { id_token, access_token, scope, expires_at, refresh_token, ...remainder } = tokenEndpointResponse;
  const storeIDToken = config.session.storeIDToken;

  const user = claims.user || claims;
