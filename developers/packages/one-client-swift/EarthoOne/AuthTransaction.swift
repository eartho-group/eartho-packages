...
/**
Represents an ongoing auth transaction with an identity provider (EarthoOne or a third party).
...
- Important: Only one ``AuthTransaction`` can be active at a given time for EarthoOne.swift, if you start a new one before finishing the current one it will be cancelled.
*/
