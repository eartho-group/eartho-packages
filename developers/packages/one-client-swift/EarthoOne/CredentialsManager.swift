...
public struct CredentialsManager {
    ...
    ///
    /// - Parameters:
    ///   - authentication: EarthoOne Authentication API client.
...
//#if WEB_AUTH_PLATFORM
    ...
    /// - Requires: The scope `offline_access` to have been requested on login to get a refresh token from EarthoOne. If the credentials are expired and there is no refresh token, the callback will be called with a failure case containing a ``CredentialsManagerError/noRefreshToken`` error.
...
public extension CredentialsManager {
    ...
    /// - Throws: An error of type ``CredentialsManagerError``.
    /// - Requires: The scope `offline_access` to have been requested on login to get a refresh token from EarthoOne. If the credentials are expired and there is no refresh token, the error ``CredentialsManagerError/noRefreshToken`` will be thrown.
    /// - Note: This method is thread-safe.
    /// - See: [Refresh tokens](https://eartho.com/docs/secure/tokens/refresh-tokens)
    /// - See: [Authentication API Endpoint](https://eartho.com/docs/api/authentication#refresh-token)
#if compiler(>=5.5.2)
