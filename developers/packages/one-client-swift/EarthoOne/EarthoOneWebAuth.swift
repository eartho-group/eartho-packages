...
final class EarthoOneWebAuth: WebAuth {
    ...
    var state: String {
        return self.parameters["state"] ?? {
            self.parameters["state"] = self.generateDefaultState();
            return self.parameters["state"]!;
        }()
    }
    
    var nonce: String {
        return self.parameters["nonce"] ?? {
            self.parameters["nonce"] = self.generateDefaultNonce();
            return self.parameters["nonce"]!;
        }()
    }
    ...
    func accessId(_ accessId: String) -> Self {
        self.parameters["access_id"] = accessId
        return self
    }
    ...
    func buildAuthorizeURL(withRedirectURL redirectURL: URL,
                           ...
                           state: String?,
                           nonce:String?) -> URL {
        let authorize = URL(string: "connect", relativeTo: self.url)!
    ...
    func generateDefaultNonce() -> String {
        ...
        guard result == 0, let nonce = tempData.a0_encodeBase64URLSafe()
        else { return UUID().uuidString.replacingOccurrences(of: "-", with: "") }
    ...
    private func handler(_ redirectURL: URL) -> OAuth2Grant {
        ...
        return PKCE(authentication: authentication,
                    ...
                    maxAge: self.maxAge,
                    nonce: self.nonce,
                    accessId: self.parameters["access_id"]!)
    }
