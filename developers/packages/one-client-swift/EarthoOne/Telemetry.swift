...
/// Generates and sets the `EarthoOne-Client` header.
public struct Telemetry {
    ...
    static let LibraryName = "EarthoOne.swift"
    ...
    func addTelemetryHeader(request: NSMutableURLRequest) {
        if let value = self.value {
            request.setValue(value, forHTTPHeaderField: "EarthoOne-Client")
        } else {
            request.setValue(nil, forHTTPHeaderField: "EarthoOne-Client")
        }
    }
    ...
    func queryItemsWithTelemetry(queryItems: [URLQueryItem]) -> [URLQueryItem] {
        ...
        if let value = self.value {
            items.append(URLQueryItem(name: "earthoClient", value: value))
        }
        return items
    }
...
/// A type that can send the `EarthoOne-Client` header on every request to EarthoOne.
...
public extension Trackable {
    /**
     Turn on/off sending the `EarthoOne-Client` header on every request to EarthoOne.
    ...
    /**
     Includes the name and version of the library/framework which has EarthoOne.swift as dependency.
     
     - Parameter name:    Name of library or framework that uses EarthoOne.swift.
