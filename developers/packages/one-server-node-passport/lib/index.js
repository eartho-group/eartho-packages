...
function Strategy(options, verify) {
  ...
  options.customHeaders = Object.assign(
    { 'Eartho-Client': clientInfoHeader },
    typeof options.customHeaders === 'object' ? options.customHeaders : {}
  );

  var defaultOptions = {
    expectedIssuer: 'https://one.eartho.world/',
    authorizationURL: 'https://one.eartho.world/connect',
    tokenURL: 'https://api.eartho.world/access/oauth/token',
    userInfoURL: 'https://api.eartho.world/me',
...
Strategy.prototype.authenticate = function (req, options) {
  ...
  if (this.options.state) {
    if (!req.session) {
      throw new Error('EarthoStrategy requires you set state to false when no session is present')
    }
    ...
    throw new Error('Scope "openid" is not allowed without EarthoStrategy state true')
  }
...
/**
 * @deprecated An Access Token is already included as part of the verify callback passed to EarthoStrategy.
 */
...
/**
 * Retrieve user profile from Eartho.
 *
 ...
 *   - `username`         this is the displayName of the auth0 profile
 *   - `displayName`
 *
