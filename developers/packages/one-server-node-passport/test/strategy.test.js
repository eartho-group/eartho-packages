var EarthoStrategy = require('../lib');
...
describe('auth0 strategy', function () {
  beforeEach(function () {
    this.strategy = new EarthoStrategy({
       domain:       'test.eartho.world',
       clientID:     'testid',
  ...
  describe('options', function() {
    describe('defaults', function() {
      it('expectedIssuer should have the domain', function () {
        ...
        .expectedIssuer.should.eql('https://test.eartho.world/');
      });

      it('authorizationURL should have the domain', function () {
        this.strategy.options
          .authorizationURL.should.eql('https://test.eartho.world/authorize');
      });

      it('tokenURL should have the domain', function () {
        this.strategy.options
          .tokenURL.should.eql('https://test.eartho.world/oauth/token');
      });

      it('userInfoURL should have the domain', function () {
        this.strategy.options
          .userInfoURL.should.eql('https://test.eartho.world/userinfo');
      });

      it('apiURL should have the domain', function () {
        this.strategy.options
          .apiUrl.should.eql('https://test.eartho.world/api');
      });
    });

    it('should not override options with defaults', function() {
      const strategy = new EarthoStrategy({
          domain:       'test.eartho.world',
          clientID:     'testid',
          clientSecret: 'testsecret',
          callbackURL:  '/callback',
  ...
  it('should include a telemetry header by default', function() {
    ...
    should.exist(headers['Eartho-Client']);
  });

  it('should include a correct telemetry values', function() {
    var telemetryValue = new Buffer( this.strategy.options.customHeaders['Eartho-Client'], 'base64' ).toString('ascii');
  ...
  it('should copy options object without mutating', function () {
    ...
    var strategy = new EarthoStrategy(
      options,
      function(accessToken, idToken, profile, done) {}
    );
  ...
  describe('authorizationParams', function () {
    ...
    it('should map the login_hint field', function () {
      var extraParams = this.strategy.authorizationParams({login_hint: 'test.user@eartho.world'});
      extraParams.login_hint.should.eql('test.user@eartho.world');
    });
...
describe('auth0 strategy with a custom header', function () {
  ...
  it('should keep the telemetry header', function() {
    should.exist(strategy.options.customHeaders['Eartho-Client']);
  });
});
