...
describe('The Auth HTTP Interceptor', () => {
  ...
  beforeEach(() => {
    ...
    spyOn(earthoOneClient, 'getIdToken').and.resolveTo('access-token');
  ...
  describe('Requests that are configured using a complex object', () => {
    ...
    it('pass through the route options to getIdToken, without additional properties', fakeAsync((
      done: () => void
    ) => {
      // Testing { uri: /api/addresses } (exact match)
      assertAuthorizedApiCallTo('https://my-api.com/api/addresses', done);

      expect(earthoOneClient.getIdToken).toHaveBeenCalledWith({
        audience: 'audience',
        scope: 'scope',
      });
    }));
    ...
    it('does not execute HTTP call when not able to retrieve a token', fakeAsync((
      ...
      (earthoOneClient.getIdToken as jasmine.Spy).and.returnValue(
    ...
    it('emit error when not able to retrieve a token but allowAnonymous is set to false', fakeAsync((
      ...
      (earthoOneClient.getIdToken as jasmine.Spy).and.callFake(() => {
