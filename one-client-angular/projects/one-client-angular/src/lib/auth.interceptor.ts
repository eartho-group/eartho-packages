...
import { EarthoOne, GetTokenSilentlyOptions } from '@eartho/one-client-js';
...
export class AuthHttpInterceptor implements HttpInterceptor {
    ...
    return this.findMatchingRoute(req, config.httpInterceptor).pipe(
      concatMap((route) =>
        iif(
          ...
          () => route !== null,
          // If we have a matching route, call getIdToken and attach the token to the
          // outgoing request
          of(route).pipe(
            pluck('tokenOptions'),
            concatMap<GetTokenSilentlyOptions, Observable<string>>(
              (options) => {
                return this.connectSilently(options).pipe(
                  catchError((err) => {
  ...
  /**
   * Duplicate of AuthService.connectSilently, but with a slightly different error handling.
   * Only used internally in the interceptor.
   * @param options The options for configuring the token fetch.
   */
  private connectSilently(
    options?: GetTokenSilentlyOptions
  ): Observable<string> {
    return of(this.earthoOneClient).pipe(
      concatMap((client) => client.getIdToken(options)),
