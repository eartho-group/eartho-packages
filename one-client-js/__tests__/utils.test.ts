...
describe('utils', () => {
  ...
  describe('validateCrypto', () => {
    ...
    it('should throw error if crypto.subtle is undefined', () => {
      ...
      expect(validateCrypto).toThrowError(`
      one-client-js must run on a secure origin. See https://github.com/eartho/one-client-js/blob/main/FAQ.md#why-do-i-get-one-client-js-must-run-on-a-secure-origin for more information.
    `);
    });
  });
});
