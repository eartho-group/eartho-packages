...
export const getConfig = (params: ConfigParameters = {}): NextConfig => {
  ...
  const EARTHO_ISSUER_BASE_URL = process.env.EARTHO_ISSUER_BASE_URL || 'https://one.eartho.io';
  const EARTHO_BASE_URL = process.env.EARTHO_BASE_URL || process.env.NEXT_PUBLIC_EARTHO_BASE_URL;
