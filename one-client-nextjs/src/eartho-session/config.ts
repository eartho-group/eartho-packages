...
import { EarthoRequestCookies } from './http';
...
export interface Config {
  ...
  /**
   * Boolean value to enable Eartho's logout feature.
   */
  earthoLogout?: boolean;
   ...
   *     audience: 'https://your-eartho-api-identifier'
   *   }
   * }));
   * ```
   *
   ...
   * via the `Eartho-Client` header. Defaults to `true`.
   */
   ...
   * you can also use the `EARTHO_CLIENT_ASSERTION_SIGNING_KEY` environment variable.
   *
   ...
   * You can also use the `EARTHO_CLIENT_ASSERTION_SIGNING_ALG` environment variable.
   */
...
export interface LogoutOptions {
   ...
   *
   * @example pass the federated logout param per https://eartho.io/docs/authenticate/login/logout/log-users-out-of-idps
   *
   * ```js
...
export type GetConfig = Config | ((req: EarthoRequestCookies) => Config | Promise<Config>);
