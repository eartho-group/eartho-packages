...
export type HandleCallback = (req: EarthoRequest, res: EarthoResponse, options?: CallbackOptions) => Promise<void>;
  ...
  return async (req, res, options) => {
    ...
    try {
      tokenResponse = await client.callback(
        ...
        { exchangeBody: { ...options?.authorizationParams, client_id: config.clientID, version: 'v2' } }
      );
    } catch (err) {
      err.status = 400;
      err.statusCode = 400;
