...
import { EarthoRequestCookies, EarthoResponseCookies } from '../http';
...
export abstract class AbstractSession<Session> {
  protected getConfig: (req: EarthoRequestCookies) => Config | Promise<Config>;

  constructor(getConfig: GetConfig) {
    this.getConfig = typeof getConfig === 'function' ? getConfig : () => getConfig;
  }

  abstract getSession(req: EarthoRequestCookies): Promise<SessionPayload<Session> | undefined | null>;

  abstract setSession(
    req: EarthoRequestCookies,
    res: EarthoResponseCookies,
    session: Session,
    uat: number,
    iat: number,
    exp: number,
  ...
  public async read(req: EarthoRequestCookies): Promise<[Session?, number?]> {
