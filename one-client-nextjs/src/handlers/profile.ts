...
import { EarthoNextApiRequest, EarthoNextRequest } from '../http';
...
const appRouteHandlerFactory: (
  ...
  async (req, _ctx, options = {}) => {
    try {
      const config = await getConfig(new EarthoNextRequest(req));
...
const pageRouteHandlerFactory: (
  ...
  async (req: NextApiRequest, res: NextApiResponse, options = {}): Promise<void> => {
    try {
      ...
      const config = await getConfig(new EarthoNextApiRequest(req));
      const client = await getClient(config);
