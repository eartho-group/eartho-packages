...
export interface AccessTokenRequest {
   ...
   * import { getAccessToken } from '@eartho/one-client-nextjs';
   *
 ...
 * // app/my-api/route.js
 * import { NextResponse } from 'next/server';
 * import { getAccessToken } from '@eartho/one-client-nextjs/edge'; // Note the /edge import
 *
 * export async function GET() {
 *   const { accessToken } = await getAccessToken();
 *   return NextResponse.json({ foo: 'bar' });
...
export default function accessTokenFactory(
  ...
  return async (reqOrOpts?, res?, accessTokenRequest?): Promise<GetAccessTokenResult> => {
    ...
    const config = await getConfig(req ? getEarthoReqRes(req, res as any)[0] : new EarthoNextRequestCookies());
