...
import { SessionCache, Session, get } from '.';
 ...
 * ```js
 * // app/my-api/route.js
 * import { getSession } from '@eartho/one-client-nextjs';
 *
 ...
 * // using another part of the SDK like `withServerAccessRequired` or `getAccessToken`.
 ...
 * ```js
 * // app/my-api/route.js
 * import { getSession } from '@eartho/one-client-nextjs/edge'; // Note the /edge import
 *
 * export default async function MyPage({ params, searchParams }) {
 *   const { user } = await getSession();
 *   return <h1>My Page</h1>;
