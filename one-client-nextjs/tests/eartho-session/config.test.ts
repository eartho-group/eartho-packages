import { Config, ConfigParameters, getConfig, AuthorizationParameters, DeepPartial } from '../../src/eartho-session';
...
describe('Config', () => {
  ...
  it('should set idpLogout to true when earthoLogout is true', () => {
    const config = getConfig({
      ...defaultConfig,
      earthoLogout: true
    });
  ...
  it('earthoLogout should default to undefined and idpLogout should default to false', () => {
    const config = getConfig(defaultConfig);
    expect(config.idpLogout).toBe(false);
    expect(config.earthoLogout).toBeUndefined();
  });

  it('should not set earthoLogout to true when idpLogout is true', () => {
    const config = getConfig({
      ...defaultConfig,
      idpLogout: true
    });
