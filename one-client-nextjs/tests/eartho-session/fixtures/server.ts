...
import {
  ...
  StatefulSession,
  AbstractSession
} from '../../../src/eartho-session';
import wellKnown from './well-known.json';
import { jwks } from './cert';
import { cert, key } from './https';
import { Claims } from '../../../src/session';
import version from '../../../src/version';
import { NodeRequest, NodeResponse } from '../../../src/eartho-session/http';
import { clientGetter } from '../../../src/eartho-session/client/node-client';
import backchannelLogoutHandlerFactory from '../../../src/eartho-session/handlers/backchannel-logout';
...
const createHandlers = (params: ConfigParameters): Handlers => {
  ...
  const getClient = clientGetter({ name: 'nextjs-eartho', version });
...
export const setup = async (
  ...
  nock('https://test.eu.eartho.io', { allowUnmocked: true }).persist().get('/.well-known/jwks.json').reply(200, jwks);
