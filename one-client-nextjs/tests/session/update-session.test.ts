...
import { get, post } from '../eartho-session/fixtures/helpers';
      ...
      const res = await getResponse({
        ...
        extraHandlers: {
          async update(req: NextRequest) {
            ...
            await earthoInstance.updateSession(req, res, { ...session, user: { ...session?.user, foo: 'bar' } });
            return res;
          }
        }
      });
    ...
    test('should update session from a server component', async () => {
      ...
      const res = await getResponse({
        ...
        extraHandlers: {
          async update() {
            // const res = new NextResponse();
            const session = await earthoInstance.getSession();
            await earthoInstance.updateSession({ ...session, user: { ...session?.user, foo: 'bar' } });
