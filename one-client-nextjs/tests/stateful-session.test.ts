...
import { get, toSignedCookieJar, Store } from './eartho-session/fixtures/helpers';
import { setup, teardown, login } from './fixtures/setup';
import { SessionPayload } from '../src/eartho-session/session/abstract-session';
import { makeIdToken } from './eartho-session/fixtures/cert';
import { ConfigParameters, SessionStore } from '../src/eartho-session';
...
describe('next stateful session', () => {
  ...
  it('should provide current user session to custom session id generator', async () => {
    const genId = jest.fn().mockImplementation((_req, session) => session.user.displayName);
    ...
    expect(cookieJar.getCookieStringSync(baseURL)).toMatch(/^appSession=__test_displayName__\..+/);
  });
});
