...
/// Web-based authentication using EarthoOne.
///
/// - See: ``WebAuthError``
/// - See: [Universal Login](https://eartho.com/docs/authenticate/login/eartho-universal-login)
public protocol WebAuth: Trackable, Loggable {
    ...
    /**
     Specify an EarthoOne connection to directly show that identity provider's login page, skipping the Universal Login
    ...
    /**
     Specify the accessId that will be requested during authentication.


     */
    func accessId(_ accessId: String) -> Self
    ...
    /**
     ...
     - Returns: The same `WebAuth` instance to allow method chaining.
     - See: [Connection Scopes](https://eartho.com/docs/authenticate/identity-providers/adding-scopes-for-an-external-idp)
     */
    ...
    ///  authentication.
    ///  This value must match the **API Identifier** defined in the APIs section of the [EarthoOne Dashboard](https://manage.eartho.com/#/apis).
    ///
    /// - Parameter audience: Audience value. For example, `https://example.com/api`.
    /// - Returns: The same `WebAuth` instance to allow method chaining.
    /// - See: [Audience](https://eartho.com/docs/secure/tokens/access-tokens/get-access-tokens#control-access-token-audience)]
    ...
    /// This value will be used instead of the EarthoOne Domain.
    ///
    ...
    /// - See: [FAQ](https://github.com/eartho/EarthoOne.swift/blob/master/FAQ.md)
    ...
    /**
     ...
     ```
     EarthoOne
         .webAuth(clientId: clientId, domain: "samples.eartho.com")
         .start { result in
             switch result {
     ...
     - Requires: The **Callback URL** to have been added to the **Allowed Callback URLs** field of your EarthoOne application settings in the [Dashboard](https://manage.eartho.com/#/applications/).
     */
    ...
    /**
     ...
     ```
     do {
         let credentials = try await EarthoOne
             .webAuth(clientId: clientId, domain: "samples.eartho.com")
             .start()
         print("Obtained credentials: \(credentials)")
     } catch {
    ...
    /**
     Removes the EarthoOne session and optionally removes the identity provider (IdP) session.
     ...
     Remove both the EarthoOne session and the identity provider session:
     ...
     - Requires: The **Callback URL** to have been added to the **Allowed Logout URLs** field of your EarthoOne application settings in the [Dashboard](https://manage.eartho.com/#/applications/).
     ...
     - See: [Logout](https://eartho.com/docs/authenticate/login/logout)
     */
